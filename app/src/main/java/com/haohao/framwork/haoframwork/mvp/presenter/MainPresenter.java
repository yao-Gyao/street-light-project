...
public class MainPresenter extends BaseFragmentPresenter {

    private ExpressView expressView;
    private DataManager dataManager;

    public MainPresenter(ExpressView expressView, LifecycleProvider<FragmentEvent> provider) {
        super(provider);
    ...
    public void getList() {
        ...
        Observable<MainListBean> expressInfo = dataManager.getList();
        expressInfo.subscribeOn(Schedulers.io()) // 在子线程中进行Http访问
                .observeOn(AndroidSchedulers.mainThread()) // UI线程处理返回接口
                .compose(getProvider().<MainListBean>bindUntilEvent(FragmentEvent.DESTROY)) // onDestroy取消订阅
