...
public abstract class Utils {
    ...
    public static int calcTextHeight(Paint paint, String demoText) {
        ...
        //
        if (demoText.contains("\n")) {
            String[] split = demoText.split("\n");
            if (split != null && split.length > 0) {
                return (int) (r.height() * split.length * 1.5f);
            }
        }
        return r.height();
    }
            ...
            if (text.contains("\n")) {
                ...
                if (split != null && split.length > 0) {
                    for (int i = 0; i < split.length; i++) {
                        float w = 0;
                        if (i == 0) {
                            w = (Utils.calcTextWidth(paint, text) / text.length() * (text.length() - split[i].length())) / 2;
                        } else {
                            w = Utils.calcTextWidth(paint, split[i]) / 2 -4;
                        }
                        c.drawText(split[i], drawOffsetX + w, drawOffsetY + paint.getTextSize() * i, paint);
                    }
                }
            } else {
                c.drawText(text, drawOffsetX, drawOffsetY, paint);
